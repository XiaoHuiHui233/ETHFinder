...
from .sqrtprice import get_next_sqrt_price_from_input, get_next_sqrt_price_from_output

def compute_swap_step(sqrt_ratio_current_X96: int,
        sqrt_ratio_target_X96: int,
        liquidity: int, amount_remaining: int,
        fee_pips: int) -> tuple[int, int, int, int]:
    zero_for_one = sqrt_ratio_current_X96 >= sqrt_ratio_target_X96
    exact_in = amount_remaining >= 0
    if exact_in:
        amount_remaining_less_fee = mul_div(amount_remaining, 1e6 - fee_pips, 1e6)
        if zero_for_one:
    ...
    if zero_for_one:
        ...
        if exact_in and sqrt_ratio_next_X96 != sqrt_ratio_target_X96:
            ...
            fee_amount = mul_div_rounding_up(amount_in, fee_pips, 1e6 - fee_pips)
