...
def get_amount0_delta(
    ...
    if round_up:
        ...
        return mul_div(
            numerator1, numerator2, sqrt_ratio_B_X96
        ) // sqrt_ratio_A_X96
...
def get_amount0_delta_with_signed(
    sqrt_ratio_A_X96: int, sqrt_ratio_B_X96: int, liquidity: int
) -> int:
    if liquidity < 0:
        return -get_amount0_delta(
            sqrt_ratio_A_X96, sqrt_ratio_B_X96, -liquidity, False
        )
    else:
        return get_amount0_delta(
            sqrt_ratio_A_X96, sqrt_ratio_B_X96, liquidity, True
        )


def get_amount1_delta_with_signed(
    sqrt_ratio_A_X96: int, sqrt_ratio_B_X96: int, liquidity: int
) -> int:
    if liquidity < 0:
    ...
    if add:
        ...
        if amount <= 2**160 - 1:
            ...
            quotient = mul_div_rounding_up(
                amount, 0x1000000000000000000000000, liquidity
            )
